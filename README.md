...
一个克制、编辑型的个人作品集网站，方向是 AI 产品观察 / 技术转译 / 选题策划。观察笔记已上线，作品索引待后续补充。
...
2. 在 `src/content/works/<slug>.js` 中创建同名文件，写入 `detailSections`。
...
如需在作品中展示图片，可把图片放入 `public/content-assets/works/<slug>/`，并在对应 content 模块中扩展 `attachments` 字段，由组件渲染。

### 10. 视觉与样式

- 全局样式集中在 `src/styles/main.css`，可从 `:root` 调整背景、文字、分割线和 accent 色。
- 视觉图在 `public/assets/signal-field.png`，可以替换为自己的研究图、内容地图或项目视觉。
